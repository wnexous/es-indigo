...
export default ({ ...props }: PendingListI) => {
    ...
    useEffect(() => {
        apiFrontend.request.getApprovedProofs().then(({ data, metadata }) => {
            setProofs(data)
    ...
    return <>
        ...
        <OrderList filterPlaceholder="Pesquisar por email" itemTemplate={itemTemplate} dataKey="id" value={proofs} header="Aprovados" filter filterBy="user.email"></OrderList>
        <ProofModal
            data={modalInfo!}
